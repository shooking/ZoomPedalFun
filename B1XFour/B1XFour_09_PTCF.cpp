...
vector<BYTE> unpack ( vector<BYTE> &sysex )
{
	...
	// We expect this unpacked sysex to start from byte 10 (0 bias)
	for (size_t i = 10; i < sysex.size() - 1; i++)
	{	
		//byte in packet:
		uint8_t byt = sysex[i];
		if (loop != -1)
	...
	*/
	...
	vector<char> patch_name;
	pair	<int, int> fx_gid_pair;
	vector< pair<int, int> > fx_gid;
	...
	if (PTCFstart != -1) 
	{
		...
		{
			printf("%c", unpacked[j]);
			patch_name.push_back(unpacked[j]);
		}
		printf("\n");
		for (int i=0; i<4; i++)
		{
		...
		for (int jj = 0; jj < numFX; jj++)
		{
			...
			*/
			printf("FX: %d (%08x)\tFXID: %4d (%04x)\tFXGP: %4d (%04x)\n",
				...
				a.s.id / 32,
				a.s.id / 32);
			fx_gid_pair.first = a.s.fx;
			fx_gid_pair.second = a.s.id / 32;

			fx_gid.push_back(make_pair(a.s.fx, a.s.id / 32));
		}
	}
	...
	if (TXE1start != -1) 
	{
		...
		for (int i=0; i < a.x; i++)
		{
			...
			description.push_back(unpacked[TXE1start + 4 + i]);
		}
		printf("\n");
	}
	...
	*/
	...
	for (size_t s = 0; s < fx_gid.size(); s++)
	{
		cout << " is " << fx_gid[s].first << " " << fx_gid[s].second << endl;
	}
	return unpacked;
}
